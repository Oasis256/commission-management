...
@section('title', 'Admin Update Brands')
        ...
        <div class="row">
          <div class="col-12">
            <div class="card">
              ...
              <!-- /.card-header -->
              <div class="card-body">
                <form class="form-horizontal" method="post" action="{{ route('brand.update',$editBrand->id) }}" enctype="multipart/form-data">
                    @csrf
                    @method('put')
                    <input type="hidden" value="{{ $editBrand->brand_image  }}" name="oldimage">
                  <div class="form-group row">
                      <label for="brand_name" class="col-sm-2 col-form-label">Brand Name</label>
                      <div class="col-sm-10">
                      <input type="text" class="form-control" name="brand_name" id="brand_name" value="{{ $editBrand->brand_name }}">
                      @error('brand_name')
                  ...
                  <div class="form-group row">
                    ...
                    <select class="form-control" name="status">
                        ...
                        <option value="1" {{ $editBrand->status == 1 ? 'selected':'' }}>Active</option>
                        <option value="0" {{ $editBrand->status == 0 ? 'selected':'' }}>In Active</option>
                    </select>
                    @error('status')
                        <span class="invalid-feedback" style="display:block">
                            <strong class="text-danger">{{ $message }}</strong>
                  ...
                  <div class="form-group row">
                      ...
                      <div class="col-sm-10">
                          ...
                          src="{{ (!empty($editBrand->brand_image)) ? url($editBrand->brand_image) : url('upload/admin/no-image-box.png') }}" alt="brand" name="image" style="width:100px; height:100px;">
                      </div>
                    </div>     
