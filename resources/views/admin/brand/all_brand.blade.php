          ...
          <div class="col-12">
            <div class="card">
              <div class="card-header" style="background:#1f2d3d">
                <div class="row">
                    <div class="col-md-6">
                        <h3 class="card-title mt-1 text-light"> <strong>All Brand Information</strong> </h3>
                    </div>
                    <div class="col-md-6 text-right">
                        <a href="{{ route('brand.create') }}" class="btn btn-primary btn-sm"><i class="fas fa-plus-square mr-1"></i> Add Brand</a>
                    </div>
                </div>
              </div>
              <!-- /.card-header -->
              <div class="card-body">
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <tr>
                    ...
                    <td>
                        {{-- <img src="{{ asset($brand->brand_image) }}" alt="brand_image" style="width:70px; height:50px;"> --}}
                        <img class="img-fluid " id="showImage"
                        src="{{ (!empty($brand->brand_image)) ? url($brand->brand_image) : url('upload/admin/no-image-box.png') }}" alt="User" name="image" style="width:70px; height:50px;">
                    </td>
                    <td>{{ $brand->created_at->diffForHumans() }}</td>
                    <td>
                        <a href="{{ route('brand.edit',$brand->id) }}" class="btn btn-sm btn-info"><i class="fas fa-pencil-alt">
                        </i> Edit</a>
                         <a href="{{ route('brand.delete',$brand->id) }}" class="btn btn-sm btn-danger" id="delete"><i class="fas fa-trash"></i> Delete</a>
                    </td>
                  </tr> 
                  @endforeach
                  </tbody>
